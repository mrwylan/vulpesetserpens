...
**Stage 3 — Musical duration scoring via autocorrelation**

From the surviving `(S, E)` pairs, compute the loop duration `D = E - S` in samples. Apply autocorrelation over the full sample buffer at lag `D` to produce a similarity score: how closely the waveform starting at `S` matches the waveform starting at `E` (i.e., how "periodic" the signal is at that lag). Candidates where the autocorrelation score at lag `D` exceeds a threshold are scored higher. Candidates whose duration in seconds corresponds to likely musical periods (given an estimated tempo, or simply to power-of-two multiples of a detected repeat period) receive an additional musical duration bonus.

The final ranked list orders candidates by a weighted combination of: amplitude continuity score, slope match quality, and autocorrelation similarity. The top N candidates (configurable, default 10) are returned to the UI.
...
- The candidate object type is: `{ startSample: number, endSample: number, durationSeconds: number, score: number, slopeScore: number, shapeScore: number, periodScore: number, energyScore: number, crossfadeDuration: number, rank: number }`. The four core fields are required; the additional sub-scores are required for UI display and debugging.
...
- All loops are constrained to a minimum duration. Loops shorter than 0.5 seconds are discarded as musically unusable regardless of their click-free score.
