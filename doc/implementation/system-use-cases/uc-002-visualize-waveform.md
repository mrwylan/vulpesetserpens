...
> **Musician note:** Duration in decimal seconds alone is uninformative for most production decisions. If a tempo reference has been entered by the user (see UC-006), the metadata line should additionally show the approximate bar/beat count at that tempo — e.g., "4.32 s · 44100 Hz · Stereo · ≈ 2 bars @ 120 BPM". Even without a user-entered tempo, displaying duration as `mm:ss.ms` (e.g., "0:04.320") is more immediately readable than a plain decimal seconds value, because musicians read time this way from DAW transport displays.
